...
gatewayCommandList = {'getDeviceStatus':'\x00', 'gatewayBusy':'\x02', 'gatewayReady':'\x04', 
                      'turnDeviceOn':'\x06', 'turnDeviceOff':'\x08', 'setBrightness': '\x0A', 
                      'getBrightness': '\x0C', 'setColorTemperature':'\x0E', 'getColorTemperature':'\x10'}

adapterCommandList = {'deviceIsOn':'\x07', 'deviceIsOff':'\x09',
                      'brightnessLevel':'\x0D', 'colorTemperatureLevel' : '\x11'}
...
class BT_Adapter:
    ...
    def __init__(self, MAC, Gateway_MAC):
        ...
        self.color_temp_level_char = None
        self.recv_brightness_level = None
        self.recv_color_temp_level = None
    ...
    def ble_notification_handle(self, cHandle, data):
        ...
        if self.recv_command != adapterCommandList['colorTemperatureLevel']:
        ...
        if self.recv_command != adapterCommandList['brightnessLevel']:
            color_temp_level  = self.color_temp_level_char.read()[::-1]
            print 'Color temp level ascii:', binascii.b2a_hex(color_temp_level)
            print 'Brightness level original: %d' %(ord(color_temp_level))
            self.recv_color_temp_level = ord(color_temp_level)
    ...
    def handle_adapter_command(self):
        ...
        if self.recv_command == adapterCommandList['deviceIsOn']: 
            ...
            self.light_device.set_color_temp(self.recv_color_temp_level)
        ...
        elif self.recv_command == adapterCommandList['colorTemperatureLevel']:
            self.light_device.set_color_temp(self.recv_color_temp_level)
    ...
    def enable_notification(self):
        ...
        while retry > 0:
            try:
                self.command_char = self.peripheral.getCharacteristics(uuid = BT_Adapter.config['Dual_Dimmer_Command_buffer_UUID'])[0]
                self.brightness_level_char = self.peripheral.getCharacteristics(uuid = BT_Adapter.config['Brightness_Level_UUID'])[0]
                self.color_temp_level_char = self.peripheral.getCharacteristics(uuid = BT_Adapter.config['Color_Temperature_Level_UUID'])[0]
    ...
    def check_device_actions(self):
        global gatewayCommandList
        new_action = self.light_device.get_new_action()
        new_brightness = self.light_device.get_new_brightness()
        new_color_temp = self.light_device.get_new_color_temp()
        if new_action is not None:
            ...
            if new_action == 'setOn':
                ...
                if new_brightness >= 0 and new_brightness <= 100:
                    self.send_command(command = gatewayCommandList['setBrightness'], brightness = new_brightness)
                else:
                    print 'Invalid brightness value'
            elif new_action == 'getBrightnessLevel':
                self.send_command(gatewayCommandList['getBrightness'])
            elif new_action == 'setColorTemperature':
                if new_color_temp >= 0 and new_color_temp <= 100:
                    self.send_command(command = gatewayCommandList['setColorTemperature'], colortemp = new_color_temp)
                else:
                    print 'Invalid color temperature value'
            elif new_action == 'getColorTemperature':
                self.send_command(gatewayCommandList['getColorTemperature'])
            
            self.light_device.new_action = None
        
    def send_command(self, command, brightness=None, colortemp=None):        
        ...
        with self.lock:
            ...
            try:
                ...
                if brightness is not None:
                    ...
                    #print 'Sending brightness level', ord(brightness)
                    print 'Sending color temperature level', colortemp
                    self.color_temp_level_char.write(chr(colortemp), withResponse=True)
                    #self.address_mask_char.write("\x00\x00\x00\x00\x00\x00\x00\x01", withResponse=False)
            except BTLEException as e:
