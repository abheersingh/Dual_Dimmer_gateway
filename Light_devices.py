...
class Light_device:
    ...
    def __init__(self, device_id, Gateway_MAC, adapter_MAC):
        ...
        self.color_temp_level = 0
        self.new_brightness_level = None
        self.new_color_temp_level = None
    ...
    def add_device_to_cloud(self):
        ...
        if device_already_exists == 0:
            ...
            # create an instance of the API class
            api_instance = artikcloud.DevicesApi()
            device = artikcloud.Device(uid = Light_device.config['userID'], dtid = Light_device.config['deviceTypeID'], name = "Dual Dimmer", manifest_version_policy = "LATEST") # Device | Device to be added to the user
            #print(device)
            try: 
                # Add Device
    ...
    def send_device_data(self):
        ...
        device_message["colorTemperature"] = self.color_temp_level
    ...
    def get_new_color_temp(self):
        return self.new_color_temp_level
    ...
    def set_color_temp(self, new_color_temp):
        self.color_temp_level=new_color_temp
    ...
    def handle_actions(self, actions):
        ...
        """
        simple_actions = ['setOn', 'setOff', 'getBrightnessLevel', 'getColorTemperature']
        for action in actions:
            ...
            elif action['name'] == 'setColorTemperature':
                print action['name']
                print action['parameters']['level']
                self.new_action = action['name']
                self.new_color_temp_level = action['parameters']['level']
            else:
