...
from Bluetooth_Adapters import BT_Adapter
    ...
    def scan_and_connect(self):
        ...
        for dev in devices:
            ...
            for (adtype, desc, value) in dev.getScanData():
                ...
                if desc == 'Complete Local Name' and value == 'Dual Dimmer Adapter':
                    print "  %s = %s" % (desc, value)
                    try:
                        dev_conn = BT_Adapter(dev.addr, gateway.GATEWAY_MAC)
                        dev_svc=dev_conn.peripheral.getServiceByUUID(BT_Adapter.config['Dual_Dimmer_Service_UUID'])
                        print "Service:"                
                        print(dev_svc)
                        print dev_conn  
